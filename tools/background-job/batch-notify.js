...
async function queueSignalMessage({ bridgeDir = "", env = process.env, message, queueDir = "", recipient = "" }) {
  ...
  const requestId = `batch-${Date.now()}-${Math.random().toString(16).slice(2, 10)}`;
  const pendingDir = path.join(resolvedQueueDir, "pending");
  await fsp.mkdir(pendingDir, { recursive: true });
  await fsp.mkdir(path.join(resolvedQueueDir, "results"), { recursive: true });
  const payload = {
    files: [],
    id: requestId,
  ...
  await writeJson(path.join(pendingDir, `${requestId}.json`), payload);
  return {
    id: requestId,
    payload,
